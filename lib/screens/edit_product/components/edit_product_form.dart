...
class _EditProductFormState extends State<EditProductForm> {
  ...
  void initState() {
    ...
    if (widget.product == null) {
      ...
      productDetails.initialSelectedImages = widget.product!.images!.map((e) => CustomImage(imgType: ImageType.network, path: e ?? '')).toList();

      // productDetails.initialProductType = product!.productType;
      productDetails.initSearchTags = product!.searchTags! as List<String>;
    }
  }
    ...
    return Consumer<ProductDetails>(
      builder: (context, productDetails, child) {
        ...
        /*return Tags(
          ...
          heightHorizontalScroll: getProportionateScreenHeight(200),
            ...
            return Row(
              ...
              children: [
                ...List.generate(
                  ...
                  (index) => Container(
                    decoration: BoxDecoration(borderRadius: BorderRadius.circular(25), border: Border.all(color: kSecondaryColor)),
                    ...
                    child: Padding(
                      ...
                      child: GestureDetector(
                        ...
                        child: productDetails.selectedImages[index].imgType == ImageType.local
                            ? Image.memory(
                                File(productDetails.selectedImages[index].path).readAsBytesSync(),
                                fit: BoxFit.contain,
                              )
                            : Image.network(
  ...
  Widget buildDescriptionField() {
    return TextFormField(
      ...
      keyboardType: TextInputType.multiline,
      decoration: InputDecoration(
        hintText: "e.g., This a flagship phone under made in India, by Samsung. With this device, Samsung introduces its new F Series.",
  ...
  Widget buildOriginalPriceField() {
    return TextFormField(
      ...
      decoration: InputDecoration(
        hintText: "e.g., 1500",
        labelText: "Original Price (in JOD)",
        floatingLabelBehavior: FloatingLabelBehavior.always,
      ),
      validator: (_) {
  ...
  Widget buildDiscountPriceField() {
    return TextFormField(
      ...
      decoration: InputDecoration(
        hintText: "e.g., 1099",
        labelText: "Discount Price (in JOD)",
        floatingLabelBehavior: FloatingLabelBehavior.always,
      ),
      validator: (_) {
  ...
  Future<void> saveProductButtonCallback(BuildContext context) async {
    ...
    try {
      (product as Product).productType = productDetails.productType;
      (product as Product).searchTags = productDetails.searchTags;
      ...
      await showDialog(
        ...
        builder: (context) {
          return AsyncProgressDialog(
            productUploadFuture,
            message: Text(newProduct ? "Uploading Product" : "Updating Product"),
          );
        },
      );
      if (productId != null) {
    ...
    List<String> downloadUrls = productDetails.selectedImages.map((e) => e.path).toList();
    bool productFinalizeUpdate = false;
    try {
      final updateProductFuture = ProductDatabaseHelper().updateProductsImages(productId!, downloadUrls as List<String>);
      ...
      if (productDetails.selectedImages[i].imgType == ImageType.local) {
        ...
        try {
          final imgUploadFuture = FirestoreFilesAccess()
              .uploadFileToPath(File(productDetails.selectedImages[i].path), ProductDatabaseHelper().getPathForProductImage(productId, i));
