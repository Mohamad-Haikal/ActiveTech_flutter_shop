...
class ProductDatabaseHelper {
  ...
  Future<List<String>> searchInProducts(String? query, {required ProductType? productType}) async {
    Query queryRef;
    if (productType == null) {
  ...
  Future<List<Product>> getProductsWithID(List<String> productsIds) async {
    var productsRef = firestore.collection(PRODUCTS_COLLECTION_NAME);
    List<Product> productList = [];

    for (var productId in productsIds) {
      final docSnapshot = await productsRef.doc(productId).get();
      if (docSnapshot.exists) {
        productList.add(Product.fromMap(docSnapshot.data()!, id: docSnapshot.id));
      }
      // productList.add(Product('deleted'));
    }
    return productList;
  }
  ...
  Future<List<String>> get usersProductsList async {
    ...
    List<String> usersProducts = <String>[];
